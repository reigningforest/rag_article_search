...
class TestModelLoader:
    ...
    def test_load_embedder(self, mock_cached_embeddings, mock_file_store, mock_hf_embeddings, mock_pinecone):
    ...
    def test_load_all_components(
        ...
        assert result == (mock_splits, mock_index, mock_llm, mock_embedder, mock_gemini_model)
...
class TestComponentIntegration:
    ...
    def test_embedder_integration(self):
        """Test embedder integration with mock data."""
        ...
        doc_embeddings = mock_embedder.embed_documents(docs)
        assert len(doc_embeddings) == 1
        assert len(doc_embeddings[0]) == 5
    
    def test_gemini_model_integration(self):
        """Test Gemini model integration with mock responses."""
        # Create a mock Gemini model
        mock_gemini_model = Mock()
        mock_response = Mock()
        mock_response.text = "Machine learning is a field of artificial intelligence."
        ...
        assert response.text == "Machine learning is a field of artificial intelligence."
    
    def test_pinecone_index_integration(self):
        """Test Pinecone index integration with mock data."""
        # Create a mock Pinecone index
        mock_index = Mock()
        mock_matches = [
            Mock(id='doc1', score=0.95, metadata={'text': 'Document 1'}),
            Mock(id='doc2', score=0.85, metadata={'text': 'Document 2'})
        ]
        mock_results = Mock()
        ...
        results = mock_index.query(vector=query_vector, top_k=top_k, include_metadata=True)
        
        assert len(results.matches) == 2
        assert results.matches[0].id == 'doc1'
        assert results.matches[0].score == 0.95
        assert results.matches[1].id == 'doc2'
        assert results.matches[1].score == 0.85
